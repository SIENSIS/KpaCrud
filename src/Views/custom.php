...
<div class="d-flex justify-content-end">
  ...
  ?>
    ...
    <?php if ($config['add_button'] == true) : ?>
      &nbsp;<a href="<?php echo base_url($_route . '?add=item') ?>" id='custom-btn-add' class="btn btn-info" title="<?= lang('crud.help.btnAdd') ?>"><?= lang('crud.toolbars.btnAdd'); ?></a>
    <?php endif; ?>
    ...
    <?php if ($config['recycled_button'] && $config['useSoftDeletes']) : ?>
      ...
      <div class="dropdown show">
        ...
        &nbsp;<a class="btn btn-info dropdown-toggle" id='custom-btn-recycled' href="#" role="button" id="dropdownMenuLink" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?= lang('crud.toolbars.btnRecycled'); ?></a>

        <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
          <a href="<?php echo base_url($_route . '?trash=list') ?>" id='custom-btn-trash' class="dropdown-item" title="<?= lang('crud.help.btnRecycled') ?>"><?= lang('crud.toolbars.btnShowRecycled'); ?></a>
    ...
    <?php if ($config['exportXLS'] == true) : ?>
      &nbsp;<a href="<?php echo base_url($_route . '?export=xls') ?>" id='custom-btn-exportxls' class="btn btn-info" title="<?= lang('crud.help.btnExport') ?>"><?= lang('crud.toolbars.btnExport'); ?></a>
    <?php endif; ?>

    <?php if ($config['print']) { ?>
      &nbsp;<a href="#" onclick="doPrint();" class="btn btn-info" id='custom-btn-print' title="<?= lang('crud.help.btnPrint') ?>"><?= lang('crud.toolbars.btnPrint'); ?></a>
      <script>
        function doPrint() {
          var h = 700,
